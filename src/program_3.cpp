...
void program_3_begin();
void program_3_cycle();
void program_3_cycle_abort();
void program_3_cycle_close();
...
extern unsigned long time_finish_p3;
extern unsigned long time_wait_hum_p3;


void program_3_begin(){
    // Serial.println("program_3_begin");
    if (start_cycle){
        ...
        time_3_p = TIME_P3_3;
        hum_p = HUM_P3;
        delta_hum = DELTA_HUM_P3;
        delta_1_hum = DELTA_1_HUM_P3;
        delta_1_temp = DELTA_1_TEMP_P3;
        delta_2_temp = DELTA_2_TEMP_P3;
        delta_3_temp = DELTA_3_TEMP_P3;
        
        time_door_open = TIME_P3_3;
...
void program_3_cycle(){
    ...
    if(tempProb1>(temp_p)){ 
        ...
        if(hum_dht11<(hum_p*delta_1_hum)){
            ...
            if ((millis() - time_wait_hum_p3) > TIME_P0_0){
                r1.setValue(0);
                r2.setValue(0);
                r3.setValue(0);
                r0.setValue(1);
            ...
            time_finish_p3 = millis();
            time_wait_hum_p3 = millis();
             digitalWrite(OUPT1, HIGH);
